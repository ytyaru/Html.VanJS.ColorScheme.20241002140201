...
//                class: 'A BC D',
        ...
        */
        if (options.isSyncSystem ?? false) {
            this._sys = new ColorSchemeSystem({core:this._core})
            this._sys.addEvent()
        }
    }
...
//        this._onChange = ()=>{}
    ...
    get onChange() { return this._onChange }
    set onChange(v) {if ('function'!==typeof v){throw new TypeError(`onChangeは関数であるべきです。:${v}`)}else{this._onChange=v}}
    addEvent() { window.matchMedia('(prefers-color-scheme: dark)').addListener((mql)=>this.#onChange(mql)) }
    delEvent() { window.matchMedia('(prefers-color-scheme: dark)').removeListener((mql)=>this.#onChange(mql)) }
    #onChange(mql) { this._core.mode = (mql.matches) ? 'dark' : 'light' }
//    #onChange(mql) {
//        this._mode = (mql.matches) ? 'dark' : 'light'
//        this._core.mode = this._mode
//        this._onChange(this._mode)
//    }
}

class ColorSchemeCss { // dark/lightモードの具体的なCSS
    constructor(options) {
        this._core = options.core
    ...
    set() {
        ...
        root.style.setProperty('--mark-color', this._colors[this._core.mode].mark)
        root.style.setProperty('--a-link-color', this._colors[this._core.mode].a.link)
        root.style.setProperty('--a-visited-color', this._colors[this._core.mode].a.visited)
        root.style.setProperty('--a-hover-color', this._colors[this._core.mode].a.hover)
        root.style.setProperty('--a-focus-color', this._colors[this._core.mode].a.focus)
        root.style.setProperty('--a-active-color', this._colors[this._core.mode].a.active)
    }
    #injectStyle() {
        const style = document.createElement('style')
        style.name = 'color-scheme-style'
        style.textContent = `
:root {
    --base-color: white;
    --main-color: black;
...
body,input,select,textarea,label,legend {
...
a:focus {color: var(--a-focus-color);}
a:link:active, a:visited:active {color: var(--a-active-color);}
